...
void tgl_do_msg_search (struct tgl_state *TLS, tgl_peer_id_t id, int from, int to, int limit, int offset, const char *pattern, int pattern_len, void (*callback)(struct tgl_state *TLS, void *callback_extra, int success, int size, struct tgl_message *list[]), void *callback_extra) {
  ...
  E->filter = CODE_input_messages_filter_empty;
...
void tgl_do_create_group_chat (struct tgl_state *TLS, int users_num, tgl_peer_id_t ids[], const char *chat_topic, int chat_topic_len, void (*callback)(struct tgl_state *TLS, void *callback_extra, int success, struct tgl_message *M), void *callback_extra) {
  clear_packet ();
  ...
  E->multi = 0;
  E->count = 1;
  E->list = talloc (sizeof (long long));
  tglq_send_query (TLS, TLS->DC_working, packet_ptr - packet_buffer, packet_buffer, &send_msgs_methods, 0, callback, callback_extra);
}
/* }}} */
...
void tgl_do_send_audio (struct tgl_state *TLS, tgl_peer_id_t to_id, const char *file_name, int duration, void (*callback)(struct tgl_state *TLS, void *callback_extra, int success, struct tgl_message *M), void *callback_extra)
{
    _tgl_do_send_photo (TLS, to_id, file_name, 0, 100, 100, duration, 0, 0, 0, 0, TGLDF_AUDIO, callback, callback_extra);
}

static int get_invite_text_on_answer (struct tgl_state *TLS, struct query *q, void *DS) {
    struct tl_ds_help_invite_text *DS_HIT = DS;
    if (q->callback) {
        ((void (*)(struct tgl_state *, void *, int, const char *))(q->callback)) (TLS, q->callback_extra, 1, DS_HIT->message->data);
    }
    return 0;
}

static int get_invite_text_on_error (struct tgl_state *TLS, struct query *q, int error_code, int l, const char *error) {
    if (q->callback) {
...
static int delete_history_on_answer (struct tgl_state *TLS, struct query *q, void *DS) {
  struct tl_ds_messages_affected_history *DS_MAH = DS;
  ...
  if (q->callback) {
    ((void (*)(struct tgl_state *, void *, int, int))(q->callback)) (TLS, q->callback_extra, 1, *DS_MAH->offset);
  }
  return 0;
}

static int delete_history_on_error (struct tgl_state *TLS, struct query *q, int error_code, int l, const char *error) {
  if (q->callback) {
...
static int update_notify_settings_on_answer (struct tgl_state *TLS, struct query *q, void *D) {
    if (q->callback) {
        ((void (*)(struct tgl_state *TLS, void *, int))(q->callback))(TLS, q->callback_extra, 1);
    }
    return 0;
...
void tgl_do_msg_search_media (struct tgl_state *TLS, tgl_peer_id_t id, int from, int to, int limit, int offset, void (*callback)(struct tgl_state *TLS, void *callback_extra, int success, int size, struct tgl_message *list[]), void *callback_extra)
{
    ...
    E->limit = limit;
    E->offset = offset;
    char* pattern = talloc(1);
    strcpy(pattern, "");
    E->query = pattern;
    E->filter = CODE_input_messages_filter_photo_video;
    _tgl_do_msg_search(TLS, E, callback, callback_extra);
}
...
void tgl_do_msg_search_files (struct tgl_state *TLS, tgl_peer_id_t id, int from, int to, int limit, int offset, void (*callback)(struct tgl_state *TLS, void *callback_extra, int success, int size, struct tgl_message *list[]), void *callback_extra)
{
    ...
    strcpy(pattern, "");
    E->query = pattern;
    E->filter = CODE_input_messages_filter_document;
    _tgl_do_msg_search(TLS, E, callback, callback_extra);
}

static int get_wallpapers_on_answer (struct tgl_state *TLS, struct query *q, void *DS) {
    struct tl_ds_vector *DS_V = DS;
    ...
    int i;
    for (i = 0; i < n; i++) {
        struct tl_ds_wall_paper* DS_W = DS_V->f2[i];

        struct tgl_wallpaper* W = wallpapers + i;
        W->solid = DS_W->magic == CODE_wall_paper_solid ? 1 : 0;
        W->id = *DS_W->id;
        W->title = DS_W->title->data;
        if (DS_W->magic == CODE_wall_paper)
        {
            W->sizes_num = *DS_W->sizes->cnt;
            W->sizes = talloc (sizeof (struct tgl_photo_size) * W->sizes_num);
            int j;
            for (j = 0; j < W->sizes_num; j++) {
              tglf_fetch_photo_size_new(TLS, &W->sizes[j], DS_W->sizes->data[j]);
            }
        }
        else
            wallpapers[i].bg_color = *DS_W->bg_color;
        wallpapers[i].color = *DS_W->color;
    }
  if (q->callback) {
...
static int get_account_ttl_on_answer (struct tgl_state *TLS, struct query *q, void *DS) {
    struct tl_ds_account_days_t_t_l *DS_ADT = DS;
  if (q->callback) {
    ((void (*)(struct tgl_state *, void *, int, int))(q->callback)) (TLS, q->callback_extra, 1, *DS_ADT->days);
  }
  return 0;
}

static int get_account_ttl_on_error (struct tgl_state *TLS, struct query *q, int error_code, int l, const char *error) {
  if (q->callback) {
    ((void (*)(struct tgl_state *, void *, int, int))(q->callback)) (TLS, q->callback_extra, 0, 0);
  }
  return 0;
...
static int set_account_ttl_on_answer (struct tgl_state *TLS, struct query *q, void *D) {
  if (q->callback) {
    ((void (*)(struct tgl_state *, void *, int))(q->callback)) (TLS, q->callback_extra, 1);
  }
  return 0;
